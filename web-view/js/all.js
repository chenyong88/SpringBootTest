...
/*!
 * ZUI: zui for ranzhi - v1.7.0 - 2017-11-07
 * http://zui.sexy
...
function(d, g, c) {
	...
	f.DEFAULTS = h, f.NAME = b, f.prototype.trigger = function(k, a) {
		...
		var ab, ac, W, Y, ae, ak, al, ah, aa, aj, R, ai, ad, Q = this,
			am = Q.$,
			J = Q.options,
			N = J.deviation,
			X = "." + b + "." + Q.id,
			...
			G = am,
			F = !1,
			ag = V ? d(J.container).first() : a ? am : d("body"),
			K = function(p) {
				if(F && (R = {
						left: p.pageX,
						top: p.pageY
					}, !(c.abs(R.left - ah.left) < N && c.abs(R.top - ah.top) < N))) {
					if(null === W) {
						...
						"absolute" != o && "relative" != o && "fixed" != o && (ak = o, ag.css("position", "relative")), W = G.clone().removeClass("drag-from").addClass("drag-shadow").css({
							...
							width: G.outerWidth(),
							transition: "none"
						}).appendTo(ag), G.addClass("dragging"), Q.trigger("start", {
							event: p,
							element: G
						})
					}
			...
			Z = function(m) {
				if(d(g).off(X), clearTimeout(ad), F) {
					if(F = !1, ak && ag.css("position", ak), null === W) {
						return G.removeClass("drag-from"), void Q.trigger("always", {
							event: m,
							cancel: !0
						})
					}
...
function(a) {
	...
	b.DEFAULTS = {
		...
		return c = a.extend({
			lang: a.zui.clientLang()
		}, b.DEFAULTS, this.$.data(), c)
		...
		d.droppable(a.extend({
			...
			drag: function(h) {
				if(d.find(".board.drop-in-empty").removeClass("drop-in-empty"), h.isIn) {
					var j = h.target.closest(".board").addClass("drop-in"),
						g = j.find(".board-item-shadow"),
						k = h.target;
...
function(b) {
	...
	c.prototype.show = function() {
		var g = this.element,
			...
			k = g.attr("data-target") || g.attr("data-tab");
		if(k || (k = g.attr("href"), k = k && k.replace(/.*(?=#[^\s]*$)/, "")), !g.parent("li").hasClass("active")) {
			var l = e.find(".active:last a")[0],
				h = d.Event("show." + f, {
					relatedTarget: l
				});
		...
		var m = h.find("> .active"),
			j = g && d.support.transition && m.hasClass("fade");
...
function(d, h) {
	...
	d.fn.collapse = function(e) {
		return this.each(function() {
			var g = d(this),
				h = g.data(f),
				a = d.extend({}, c.DEFAULTS, g.data(), "object" == typeof e && e);
	...
	function f(a, h, j) {
		return this.each(function() {
			var k = d(this),
				m = k.data(c),
				e = d.extend({}, b.DEFAULTS, k.data(), "object" == typeof a && a);
	...
	b.VERSION = "3.2.0", b.TRANSITION_DURATION = 300, b.BACKDROP_TRANSITION_DURATION = 150, b.DEFAULTS = {
		...
		if("undefined" == typeof l && (l = this.options.position), "undefined" != typeof l) {
			var h = this.$element.find(".modal-dialog"),
				m = Math.max(0, (d(window).height() - h.outerHeight()) / 2),
				p = "fit" == l ? 2 * m / 3 : "center" == l ? m : l;
			if(h.hasClass("modal-moveable")) {
				var j = null,
					k = this.options.rememberPos;
		...
		var j = this,
			h = j.options,
			k = j.$element.find(".modal-dialog").removeClass("modal-dragged");
		...
		this.isShown && this.options.keyboard ? d(document).on("keydown.dismiss." + c, d.proxy(function(j) {
			if(27 == j.which) {
				var h = d.Event("escaping." + c),
					k = this.$element.triggerHandler(h, "esc");
				if(void 0 != k && !k) {
					return
				}
				this.hide()
	...
	d.fn.modal = f, d.fn.modal.Constructor = b, d.fn.modal.noConflict = function() {
		...
		if(j.length) {
			var e = j.data(c) ? "toggle" : d.extend({
				remote: !/#/.test(p) && p
			}, j.data(), m.data());
...
function(u, k) {
	...
	w.DEFAULTS = {
		...
		var o = this,
			...
			C = this.$dialog,
			x = a.custom,
			e = C.find(".modal-body").css("padding", ""),
			A = C.find(".modal-header"),
		...
		ModalTrigger: w,
		modalTrigger: new w
	}), u.fn.modalTrigger = function(d, c) {
	...
	u.zui({
		...
		x && x.length || (c.data(j) ? c.trigger(".toggle." + j) : c.modalTrigger({
			show: !0
		})), c.is("a") && l.preventDefault()
	})
	...
	c.DEFAULTS = {
		...
		for(var m = this.options.trigger.split(" "), p = m.length; p--;) {
			...
			if("click" == h) {
				...
				if("manual" != h) {
					var j = "hover" == h ? "mouseenter" : "focus",
						d = "hover" == h ? "mouseleave" : "blur";
	...
	b.DEFAULTS = {
		...
		return !!h.image && void e.modalTrigger({
			...
			custom: function(k) {
				...
				var o = m.find(".modal-dialog"),
					p = d(f).width();
				d.zui.imgReady(h.image, function() {
					o.css({
						width: c.min(p, this.width)
					}), p < this.width + 30 && m.addClass("lightbox-full"), k.ready(200)
					...
					var s = d(this),
						u = m.data("group-index") + (s.hasClass("prev") ? -1 : 1),
					...
					if(a.length) {
						...
						m.addClass("modal-loading").data("group-index", u).toggleClass("lightbox-with-caption", "string" == typeof r).removeClass("lightbox-full"), m.find(".lightbox-img").attr("src", q), m.find(".caption > .content").text(r), p = d(f).width(), d.zui.imgReady(q, function() {
							o.css({
								width: c.min(p, this.width)
							}), p < this.width + 30 && m.addClass("lightbox-full"), k.ready()
						})
					}
...
function(x, p, k) {
	...
	b.prototype.update = function(h, d) {
		...
		var B = G.placement,
			D = x(G.parent),
			E = D.children(".messagers-holder." + B);
...
/*!
 * ZUI: Generated from less code - v1.7.0 - 2017-10-11
 * http://zui.sexy
 * GitHub: https://github.com/easysoft/zui.git 
 * Copyright (c) 2017 cnezsoft.com; Licensed MIT
 */
function(d) {
	...
	b.get = function(a) {
		...
		colorset: b
	}), d.zui.Color && d.extend(d.zui.Color, b)
...
function(d) {
	...
	b.DEFAULTS = {
		...
		0 === w.flexStart && w.flexEnd === w.colsLength && (w.flexStart = -1, w.flexEnd = -1), w.flexArea = w.flexStart >= 0, w.fixedRight = w.flexEnd >= 0 && w.flexEnd < w.colsLength - 1, w.fixedLeft = w.flexStart > 0, w.flexStart < 0 && w.flexEnd < 0 && (w.fixedLeft = !0, w.flexStart = w.colsLength, w.flexEnd = w.colsLength), this.data = w, this.callEvent("afterLoad", {
			data: w
		}), this.render()
		...
		var J, R, I, A, aa, e, N, G, P = d('<div class="datatable-rows">'),
			q = ad.length;
		...
		if(aF.flexArea) {
			var an, az, au, am, aE, aj, al, af = at.find(".scroll-slide"),
				ak = at.find(".datatable-span.flexarea"),
				ah = at.find(".datatable-span.fixed-left"),
				aw = at.find(".datatable-span.flexarea .table-datatable"),
				G = af.children(".bar"),
				ad = aB.id + "_scrollOffset";
		...
		if(ar.checkable) {
			var Z, V = aB.id + "_checkedStatus",
				ax = ar.checkedClass,
				aa = function() {
					var g = ap.first().find(".datatable-row"),
						a = g.filter("." + ax);
					ar.checkboxName && g.find(".check-row input:checkbox").prop("checked", !1);
					var e = {
						checkedAll: g.length === a.length && a.length > 0,
						checks: a.map(function() {
					...
					}), av.find(".check-all").toggleClass("checked", !!e.checkedAll), ar.storage && c.pageSet(V, e), aB.callEvent("checksChanged", {
						checks: e
					})
				},
				ag = function(j, h) {
		...
		if(ar.fixedHeader) {
			var ac, q, X, K = at.children(".datatable-head"),
				B = ar.fixedHeaderOffset || d(".navbar.navbar-fixed-top").height() || 0,
				aG = function() {
		...
		if(M || (M = F ? this.$headCells.filter('[data-index="' + F.index + '"]').addClass("sort-" + F.type) : this.$headCells.filter(".sort-up, .sort-down").first()), M.length) {
			...
			d.each(J, function(g, a) {
				...
				index: H,
				type: z ? "up" : "down"
			}, Q.storage && I.pageSet(E, F), this.callEvent("sort", {
				sorter: F
			})
		}
	}, b.prototype.refreshSize = function() {
		var p, m = this.$datatable,
			w = this.options,
			h = this.data.rows;
		this.data.cols;
...
function(d, h, c) {
	...
	g.DEFAULTS = {
		...
		var m = this.options.afterPanelRemoved,
			l = this.options.panelRemovingTip;
		...
		var l = this,
			a = this.options.onlyRefreshBody;
		...
		var e = this.$,
			q = this.options,
			m = "circle" === q.shadowType,
			n = q.circleShadowSize,
			a = n / 2,
			p = q.afterOrdered;
		...
		this.$.on("mousedown", ".panel-heading, .panel-drag-handler", function(U) {
			...
			var V, Q, I, X, F, H, N, G, r = d(this).closest(".panel"),
				R = r.parent(),
				l = r.closest(".row"),
				K = r.clone().addClass("panel-dragging-shadow"),
				B = r.offset(),
				M = e.offset(),
				o = l.find(".dragging-col-holder"),
				A = r.width(),
				s = r.height();
		...
		return l.id && d.zui.store && (p = d.zui.store.pageGet("zui.dashboard." + l.id + ".sizeConfig", p)), this.$.children(".row").each(function() {
			...
			n.children(":not(.dragging-col-holder)").each(function() {
				...
				var w = 0,
					...
					y = !1;
				d.each(a, function(z) {
					var s = a[z].data("row-id", r),
						B = s.children(".panel:first");
					if(x.push(B), !y) {
		...
		p.$.on("mousedown", ".resize-handle", function(J) {
			...
			var K = function(y) {
					if(B) {
						...
						var o = y.pageX,
							l = h.max(1, h.min(12, h.round(12 * (E + (o - F)) / L)));
		...
		q && (m.addClass("panel-loading").find(".panel-heading .icon-refresh,.panel-heading .icon-repeat").addClass("icon-spin"), d.ajax({
			...
			m.addClass("panel-error"), d.isFunction(p) && p.call(this, {
				result: !1,
				$panel: m
			})
		}).always(function() {
...
function(d, f, c) {
	...
	g.DEFAULTS = {
		...
		this.$.resize(d.proxy(this.initSize, this)), this.$btn.hover(function() {
			...
			stopPropagation: !0,
			drag: function(a) {
				var k = a.element.data("direction"),
					l = a.smallOffset,
					j = !1;
				switch(k) {
					case "left":
...
function(a) {
	...
	c.DEFAULTS = {
		...
		wrap: !0,
		touchable: !0
	}, c.prototype.touchable = function() {
		function g(p) {
			...
			switch(p.type) {
				case "touchstart":
					...
					break;
				case "touchend":
					var m = p.changedTouches[0].pageX - d,
						o = p.changedTouches[0].pageY - h;
					if(Math.abs(m) > Math.abs(o)) {
						f(q, m), Math.abs(m) > 10 && p.preventDefault()
					} else {
						var k = b(window);
						b("body,html").animate({
							scrollTop: k.scrollTop() - o
						}, 400)
					}
			}
		}
		...
		var p = b.Event("slide.zui.carousel", {
			relatedTarget: g[0],
			direction: u
		});
	...
	b.fn.carousel = function(d) {
		...
		var f, d = b(this),
			j = b(d.attr("data-target") || (f = d.attr("href")) && f.replace(/.*(?=#[^\s]+$)/, "")),
			k = b.extend({}, j.data(), d.data()),
...
function(d) {
	function g(m, l) {
		...
		return d.isArray(m) && (k = {}, d.each(m, function(n, a) {
			...
			a ? k[n] = d.extend({
				type: n
			}, f[n], d.isPlainObject(a) ? a : null) : k[n] = !1
	...
	j.DEFAULTS = {
			...
			d.isArray(u) || (u = [u]), d.each(u, function(s, r) {
				...
				if(r.html) {
					...
					if(d.isFunction(x.options.itemCreator)) {
						...
						r.url ? A.append(d("<a/>", {
							href: r.url
						}).text(r.title || r.name)) : A.append(d("<span/>").text(r.title || r.name))
					}
				}
		...
		var e = a.initialState,
			k = d.zui && d.zui.store && d.zui.store.enable;
		k && (this.selector = b + "::" + (a.name || "") + "#" + (this.$.attr("id") || h++), this.store = d.zui.store[a.name ? "get" : "pageGet"](this.selector, {})), "preserve" === e && (k ? this.isPreserve = !0 : this.options.initialState = e = "normal"), this.reload(a.data), k && (this.isPreserve = !0), "expand" === e ? this.expand() : "collapse" === e && this.collapse(), this.$.on("click", '.list-toggle,a[href="#"],.tree-toggle', function(o) {
			...
			l.callEvent("hit", {
				target: m,
				item: m.data()
			}), l.toggle(m), n.is("a") && o.preventDefault()
...
(function(X) {
	...
	var Y = an.helpers = {},
		...
		D = (Y.calculateScaleRange = function(T, A, F, W, w) {
			var k = 2,
				R = Math.floor(A / (1.5 * F)),
				B = k >= R,
				z = al(T),
				b = Q(T);
			z === b && (z += 0.5, b >= 0.5 && !W ? b -= 0.5 : z += 0.5);
			for(var M = Math.abs(z - b), S = O(M), L = Math.ceil(z / (1 * Math.pow(10, S))) * Math.pow(10, S), C = W ? 0 : Math.floor(b / (1 * Math.pow(10, S))) * Math.pow(10, S), g = L - C, P = Math.pow(10, S), y = Math.round(g / P);
				(y > R || R > 2 * y) && !B;) {
				if(y > R) {
	...
	an.instances = {}, an.Type = function(b, a, c) {
		...
		setScaleSize: function() {
			var M, w, A, P, m, k, F, x, p, b, C, L, B = Q([this.height / 2 - this.pointLabelFontSize - 5, this.width / 2]),
				z = this.width,
				y = 0;
			for(this.ctx.font = ac(this.pointLabelFontSize, this.pointLabelFontStyle, this.pointLabelFontFamily), w = 0; w < this.valuesCount; w++) {
				M = this.getPointPosition(w, B), A = this.ctx.measureText(D(this.templateString, {
					value: this.labels[w]
				})).width + 5, 0 === w || w === this.valuesCount / 2 ? (P = A / 2, M.x + P > z && (z = M.x + P, m = w), M.x - P < y && (y = M.x - P, F = w)) : w < this.valuesCount / 2 ? M.x + A > z && (z = M.x + A, m = w) : w > this.valuesCount / 2 && M.x - A < y && (y = M.x - A, F = w)
			}
		...
		draw: function() {
			if(this.display) {
				...
				if(af(this.yLabels, function(r, x) {
						if(x > 0) {
							var w, z = x * (this.drawingArea / this.steps),
								y = this.yCenter - z;
							if(this.lineWidth > 0) {
		...
		d.Type.extend({
			...
			drawLabel: function(E, I, B) {
				...
				if("outside" === F) {
					...
					var k = A.scaleFontSize * (A.scaleLineHeight || 1),
...
function fixTableHeader() {
	...
	function d() {
		...
		$("#fixedHeader").css({
			top: f,
			left: c.offset().left,
			width: (parseInt(c.css("width")) || c.width())
		})
	}
}
