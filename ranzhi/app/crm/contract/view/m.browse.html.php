<?php
/**
 * The browse mobile view file of contract module of RanZhi.
 *
...
<div class='heading'>
  ...
  <nav class='nav'>
    <a class='btn primary' data-display='modal' data-placement='bottom' data-remote='<?php echo $this->createLink('contract', 'create') ?>'>
...
<section id='page' class='section list-with-pager'>
  <?php $refreshUrl = $this->createLink('contract', 'browse', "mode={$mode}&orderBy=$orderBy&recTotal={$pager->recTotal}&recPerPage={$pager->recPerPage}&pageID={$pager->pageID}");?>
  <div class='box' data-page='<?php echo $pager->pageID ?>' data-refresh-url='<?php echo $refreshUrl ?>'>
    <table class='table bordered no-margin'>
      ...
      <tr class='text-center' data-url='<?php echo $this->createLink('contract', 'view', "contractID={$contract->id}");?>' data-id='<?php echo $contract->id;?>'>
...
$('#menu > a').removeClass('active').filter('[href*="' + mode + '"]').addClass('active');
if(mode == 'expire') $('#menu > a[href*=expired]').removeClass('active');
</script>
