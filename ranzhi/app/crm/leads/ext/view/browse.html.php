...
<div id='menuActions'>
  <?php commonModel::printLink('leads', 'apply', '', "<i class='icon-pencil'> </i>" . $lang->contact->apply, "class='leads-apply btn btn-primary'");?>
...
<div class='panel'>
  <table class='table table-bordered table-hover table-striped tablesorter table-data table-fixed' id='contactList'>
    <thead>
      <tr class='text-center'>
        ...
        <?php if(!empty($config->leads->sites)):?>
        <th class='w-80px'><?php commonModel::printOrderLink('register', $orderBy, $vars, $lang->leads->register);?></th>
        <th class='w-80px'><?php commonModel::printOrderLink('last', $orderBy, $vars, $lang->leads->last);?></th>
        <th class='w-80px'><?php commonModel::printOrderLink('visits', $orderBy, $vars, $lang->leads->visits);?></th>
        <?php endif;?>
    ...
    <tr class='text-center'>
      ...
      <?php if($mode == 'next'):?>
      <td class='text-left'><?php echo substr($contact->nextDate, 0, 10);?></td>
      <?php endif;?>
      ...
      <td class='text-left' title='<?php echo $contact->phone . ' ' . $contact->mobile;?>'><?php echo $contact->phone . ' ' . $contact->mobile;?></td>
      ...
      <?php if(!empty($config->leads->sites)):?>
      <td><?php echo substr($contact->register, 0, 10);?></td>
      <td><?php echo substr($contact->last, 0, 10);?></td>
      <td><?php echo $contact->visits;?></td>
      <?php endif;?>
      <td>
        <?php
        foreach(explode(',', $contact->origin) as $origin)
        {
            if(!$origin) continue;
            if(isset($config->leads->sites->$origin))
            {
                $site = json_decode($config->leads->sites->$origin);
                echo $site->name;
            }
            else
            {
