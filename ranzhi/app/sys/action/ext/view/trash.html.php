  ...
  <table class='table table-hover tablesorter table-border'>
    ...
    <tbody>
      ...
      <tr class='text-center'>
        ...
        if(isset($config->action->customModules[$action->objectType]))
        {
            $module = $config->action->customModules[$action->objectType];
            $link   = $this->createLink("{$appname}.flow", 'displayLayout', "module={$module->module}&method=view&id={$action->objectID}");
        }
        ?>
        ...
        <td class='text-left'>
        <?php 
            if(strpos(',workflowaction,workflowmenu,workflowdatasource,workflowrule,', ",$action->objectType,") !== false)
            { 
                echo $action->objectName;
            }
            else
            {
                echo html::a("javascript:$.openEntry(\"{$appname}\", \"{$link}\")", $action->objectName);
            }
        ?>
        </td>
