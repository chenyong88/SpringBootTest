<?php
/**
 * The contact mobile view file of provider module of RanZhi.
 *
...
<div class='heading'>
  <div class='title'><i class='icon muted icon-group'></i> <strong><?php echo $lang->provider->contact;?></strong></div>
  ...
  <?php foreach($contacts as $contact):?>
    <div data-contact-id='<?php echo $contact->id ?>' class='item item-contact with-avatar multi-lines'>
      ...
      <div class='content'>
        ...
        <div class='subtitle'><i class='icon icon-phone-sign'></i> <?php echo $lang->provider->phone . ': ' . html::a("tel:$contact->phone", $contact->phone)?></div>
        <?php endif; ?>
        <?php if(!empty($contact->email)): ?>
        <div class='subtitle'><i class='icon icon-envelope-alt'></i> <?php echo $lang->provider->email . ': ' . $contact->email?></div>
        <?php endif; ?>
        <?php if(!empty($contact->qq)): ?>
        <div class='subtitle'><i class='icon icon-qq'></i> <?php echo $lang->provider->qq . ': ' . $contact->qq?></div>
        <?php endif; ?>
        ...
        <div>
          ...
          <a data-remote='<?php echo $this->createLink('crm.resume',  'leave', "resumeID=$contact->resume") ?>' class='gray text-warning btn' data-display='ajaxAction' data-reset-locate='false' ><?php echo $lang->resume->leave; ?></a>
          <?php endif;?>
...
<div class='footer nav justified'>
  <a data-remote='<?php echo $this->createLink('provider', 'linkContact', "providerID=$providerID") ?>' data-display='modal' data-placement='bottom' class='strong text-link' title='{$lang->provider->linkContact}'><i class='icon-plus'></i>&nbsp;<?php echo $lang->provider->linkContact ?></a>
</div>
