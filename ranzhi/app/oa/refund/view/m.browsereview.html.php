...
<div class='heading'>
  ...
  <div class='title'>
    ...
    <div class='list dropdown-menu'>
      ...
      <a class='item item-year' href='<?php echo inlink('browseReview', "date=$year&status=$status");?>'><?php echo $year;?></a>
      <div class='list'>
        <?php foreach($monthList[$year] as $month):?>    
        <a class='item item-month' href='<?php echo inlink('browseReview', "date=$year$month&status=$status");?>'><?php echo $year . $month;?></a>
        <?php endforeach;?>
      </div>
      <?php endforeach;?>
    </div>
...
<section id='page' class='section list-with-pager'>
  <?php $refreshUrl = $this->createLink('refund', 'browsereview', "date=&status={$status}");?>
  <div class='box' data-refresh-url='<?php echo $refreshUrl;?>'>
    <table class='table bordered'>
      ...
      <tr class='text-center' data-url='<?php echo inlink('view', "refundID={$refund->id}&mode=review")?>'>
        <td><?php echo zget($users, $refund->createdBy)->realname;?></td>
...
$(function()
{
    ...
    if($status == 'reviewed')
    {
        ...
        $('#menu a[href*=' + $status + ']').addClass('active');
    }
}) 
</script>
